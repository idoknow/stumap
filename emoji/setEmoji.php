...
.btn:hover { 
	...
	function getIp()
	{
        ...
        $sql="UPDATE students SET emoji=".$_POST["emojiInput"]." WHERE id=".$_POST["nameInput"];
        if(mysqli_query($con,$sql)){
        	echo "<h1 class = 'set_callback'>".$name."设置成功!</h1>";
        	
        $filen="setEmojiEvent.temp";
		$file_content=file_get_contents($filen);
		$file_content.=$name." ".$_POST["emojiInput"].";";
		$fp0=fopen($filen,"w");
		fwrite($fp0,$file_content);
		fclose($fp0);
        }else{
        	echo "<h1 class = 'set_callback'>设置失败!</h1>";
        }
        mysqli_close($con);
	}?>
	<?php endif; ?>
